...
```
 ...
 │ image        │      │  l'API Google Gemini + vision       │      │  (JSON/SSE)   │
 └──────────────┘      │   • Web Search / Visit Website → vérifie   │      │  score,       │
                        ...
                        │  Gemini compare les sources, détecte les  │      │  correction,  │
...
2. **Le backend** utilise Google Gemini (`gemini-2.5-flash`) pour la vérification texte/URL.
   Compound dispose d'une recherche web et d'une visite de sites intégrées qui récupèrent des
   informations en temps réel. Pour les images, un modèle vision Gemini extrait d'abord le contenu
   visible, puis Compound vérifie les affirmations sur le web.
3. Gemini renvoie un objet JSON structuré : affirmations, résumé, sources, contexte et correction.
   Le backend recalcule ensuite le score final avec l'Evidence Engine et **n'accepte que les
   URLs réellement retournées par les outils web de Gemini**.
...
- **Progression de recherche** : le backend envoie au frontend les requêtes de recherche et l'étape
  de verdict après la réponse Gemini. Le endpoint SSE est conservé pour la compatibilité frontend,
  mais Google Gemini effectue la recherche serveur dans un appel principal.
...
├── backend/
│   ├── main.py             API FastAPI + appel à Gemini (web_search + web_fetch)
│   ├── requirements.txt
│   ├── .env.example
...
2. Au premier lancement, un fichier `.env` s'ouvre : collez votre `GEMINI_API_KEY`, enregistrez,
   relancez le script.
...
- la clé `GEMINI_API_KEY` reste uniquement dans le backend ;
- aucune clé Gemini n'est exposée au navigateur.

### Configuration

Copier `backend/.env.example` vers `backend/.env`, puis définir `GEMINI_API_KEY` et surtout un `AUTH_SECRET` long et aléatoire en production.

Le navigateur ne reçoit qu'un jeton de session TruthChecker. L'utilisateur n'a pas besoin de fournir une clé API Gemini.
